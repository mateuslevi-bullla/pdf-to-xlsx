...
def obter_deducoes(tables):
    ...
    for table in tables:
        for row in table:
            if iniciou_deducoes:
                if "TOTALDEDUÇÕES" in row:
                    index_total = row.index("TOTALDEDUÇÕES")
...
for i in range(0, len(data["pages"]), 2):
    ...
    cnpj_cei = obter_valor(tables, "01 - CNPJ/CNO")
    razao_social = obter_valor(tables, "02 - Razão Social / Nome")
    cpf = obter_valor(tables, "s é 1ri8e ,- UCFP)F")
    ...
    data_afastamento = obter_valor(tables, "p r2é6v i-o Data de Afastamento")
    total_bruto = obter_valor_totais(tables, "TOTALBRUTO")
    total_deducoes = obter_valor_totais(tables, "TOTALDEDUÇÕES")
    valor_liquido = obter_valor_totais(tables, "VALORLÍQUIDO")
